...
export const createMockOctokit = () => {
  ...
  const reposMocks = {
    ...
    createOrUpdateFileContents: mockFn({
      content: mockResponses.fileContent,
      commit: mockResponses.commit,
    }),
  ...
  const actionsMocks = {
    ...
    downloadWorkflowRunLogs: vi
      .fn()
      .mockResolvedValue({ url: 'https://example.com/logs', data: 'Mock log data' }),
