...
describe('Error Scenarios', () => {
  ...
  describe('Flakiness Detection', () => {
    it('should identify intermittent failures (flaky test)', async () => {
      ...
      const result1 = await getTool.handler({ owner: 'test-owner', repo: 'test-repo', path: 'README.md' });
      ...
      const result3 = await getTool.handler({ owner: 'test-owner', repo: 'test-repo', path: 'README.md' });
