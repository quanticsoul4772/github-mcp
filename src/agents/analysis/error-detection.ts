...
export class ErrorDetectionAgent extends BaseAgent {
  constructor() {
    super(
      'error-detection',
      '1.0.0',
      'Detects potential runtime errors and edge cases in code',
      {
   ...
   */
  private async analyzeNullPointerErrors(filePath: string, content: string, lines: string[]): Promise<Finding[]> {
    const findings: Finding[] = [];
   ...
   */
  private async analyzeArrayErrors(filePath: string, content: string, lines: string[]): Promise<Finding[]> {
    const findings: Finding[] = [];
    ...
    lines.forEach((line, index) => {
      ...
      if (line.includes('while') && line.includes('.length')) {
        ...
        if (whileMatch) {
          ...
          if (!loopBody.includes(`${arrayName}.pop()`) && 
   ...
   */
  private async analyzeAsyncErrors(filePath: string, content: string, lines: string[]): Promise<Finding[]> {
    const findings: Finding[] = [];
    ...
    lines.forEach((line, index) => {
      ...
      if (match && this.looksLikePromiseCall(match[1]) && 
   ...
   */
  private async analyzeTypeErrors(filePath: string, content: string, lines: string[]): Promise<Finding[]> {
    const findings: Finding[] = [];
   ...
   */
  private async analyzeResourceLeaks(filePath: string, content: string, lines: string[]): Promise<Finding[]> {
    const findings: Finding[] = [];
    ...
    lines.forEach((line, index) => {
      ...
      if (line.includes('fs.open') || line.includes('createReadStream') || line.includes('createWriteStream')) {
      ...
      if (line.includes('addEventListener') || line.includes('.on(')) {
        const hasRemoval = this.hasResourceCleanup(lines, index, ['removeEventListener', 'off', 'removeAllListeners']);
        if (!hasRemoval) {
          findings.push(this.createFinding(
            Severity.LOW,
            FindingCategory.RUNTIME_ERROR,
   ...
   */
  private async analyzeErrorHandling(filePath: string, content: string, lines: string[]): Promise<Finding[]> {
    const findings: Finding[] = [];
    ...
    lines.forEach((line, index) => {
      ...
      if (trimmedLine.includes('catch (') && (trimmedLine.includes('catch (e)') || trimmedLine.includes('catch (error)'))) {
        const catchBody = this.extractCatchBody(lines, index);
        if (!catchBody.includes('console.') && !catchBody.includes('logger.') && !catchBody.includes('throw')) {
          findings.push(this.createFinding(
            Severity.MEDIUM,
            FindingCategory.BEST_PRACTICE,
      ...
      if (trimmedLine.includes('throw ') && !trimmedLine.includes('new Error') && !trimmedLine.includes('Error(')) {
   ...
   */
  private async analyzeSecurityVulnerabilities(filePath: string, content: string, lines: string[]): Promise<Finding[]> {
    const findings: Finding[] = [];
    ...
    lines.forEach((line, index) => {
      ...
      if (line.includes('innerHTML') && (line.includes('+') || line.includes('${') || line.includes('concat'))) {
        findings.push(this.createFinding(
          Severity.HIGH,
          FindingCategory.SECURITY_VULNERABILITY,
          'Potential XSS',
  ...
  private mightBeNullish(variable: string, content: string, lines: string[], currentIndex: number): boolean {
    ...
    return nullishSources.some(pattern => 
      new RegExp(pattern).test(content) || 
      lines.slice(0, currentIndex).some(line => new RegExp(pattern).test(line))
    );
  }

  private hasArrayBoundsCheck(arrayName: string, index: string, lines: string[], currentIndex: number): boolean {
    // Look for bounds check in nearby lines
    const checkPattern = new RegExp(`${index}\\s*[<>=].*${arrayName}\\.length|${arrayName}\\.length\\s*[<>=].*${index}`);
    
    for (let i = Math.max(0, currentIndex - 3); i <= Math.min(lines.length - 1, currentIndex + 1); i++) {
  ...
  private looksLikePromiseCall(call: string): boolean {
    ...
    const promiseIndicators = [
      'fetch(', 'axios.', 'request(', 'query(', 'save(', 'update(', 'delete(',
      'create(', 'find(', 'exec(', 'then(', 'async', 'Promise'
    ];
    
    return promiseIndicators.some(indicator => call.includes(indicator));
  }

  private hasResourceCleanup(lines: string[], startIndex: number, cleanupMethods: string[]): boolean {
