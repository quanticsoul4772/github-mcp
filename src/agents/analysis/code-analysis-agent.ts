...
export class CodeAnalysisAgent extends AbstractBaseAgent {
  ...
  public async analyze(context: AnalysisContext): Promise<AnalysisResult> {
    ...
    try {
      ...
      for (const file of targetFiles) {
        ...
        if (content === null) {
          findings.push(
            this.createFinding('medium', 'file-access', `Could not read file: ${file}`, { file })
          );
          continue;
        }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (this.isJavaScriptFile(file)) {
        ...
        if (this.isMissingSemicolon(line)) {
          findings.push(
            this.createFinding('low', 'syntax', 'Missing semicolon', {
              file,
              line: lineNumber,
      ...
      if (line.includes('TODO') || line.includes('FIXME') || line.includes('HACK')) {
        findings.push(
          this.createFinding('info', 'maintenance', 'TODO/FIXME comment found', {
            file,
            line: lineNumber,
            evidence: line.trim(),
          })
  ...
  private async analyzeCodeQuality(file: string, content: string): Promise<Finding[]> {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.length > 120) {
        findings.push(
          this.createFinding('low', 'style', `Line too long (${line.length} characters)`, {
            file,
            line: lineNumber,
      ...
      if (magicNumber) {
        findings.push(
          this.createFinding('medium', 'maintainability', `Magic number found: ${magicNumber}`, {
            file,
            line: lineNumber,
      ...
      if (line.includes('catch') && lines[i + 1]?.trim() === '{' && lines[i + 2]?.trim() === '}') {
        findings.push(
          this.createFinding('high', 'error-handling', 'Empty catch block', {
            file,
            line: lineNumber + 1,
      ...
      if (line.includes('eval(') || line.includes('innerHTML') || line.includes('document.write')) {
        findings.push(
          this.createFinding('high', 'security', 'Potentially unsafe operation detected', {
            file,
            line: lineNumber,
            evidence: line.trim(),
          })
  ...
  private findMagicNumber(line: string): string | null {
    ...
    if (numbers) {
      ...
      const magicNumbers = numbers.filter(
        num => !['100', '200', '300', '400', '500', '1000'].includes(num)
      );
