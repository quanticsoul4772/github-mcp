...
export class StaticAnalysisAgent extends BaseAgent {
  constructor() {
    super('static-analysis', '1.0.0', 'Static code analysis for TypeScript and JavaScript', {
  ...
  private async analyzePatterns(
    ...
    lines.forEach((line, index) => {
      ...
      if (indentLevel > 24) {
        // More than 6 levels of nesting (assuming 4-space indents)
        findings.push(
          this.createFinding(
            Severity.MEDIUM,
            FindingCategory.MAINTAINABILITY,
  ...
  private extractFunctions(content: string): Array<{
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const functionMatch = line.match(
        /(?:function\s+([a-zA-Z_$][a-zA-Z0-9_$]*)|([a-zA-Z_$][a-zA-Z0-9_$]*)\s*[:=]\s*(?:async\s+)?(?:function|\([^)]*\)\s*=>))/
      );
