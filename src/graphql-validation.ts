...
export const ISO8601DateSchema = z
  ...
  .regex(
    ...
    'Date must be in ISO 8601 UTC format (YYYY-MM-DDTHH:mm:ss.sssZ)'
  )
  .refine(date => {
    try {
 ...
 */
export const OptionalISO8601DateSchema = z
  .string()
  .optional()
  .refine(
    date => !date || ISO8601DateSchema.safeParse(date).success,
    'Date must be in ISO 8601 format if provided'
  );
 ...
 */
export const UsernameListSchema = z
  .array(
    z
      .string()
 ...
 */
export const LabelListSchema = z
  .array(
    z
      .string()
 ...
 */
export const StarFilterSchema = z
  .string()
  .regex(
    /^(>|>=|<|<=)?\d+$|^\d+\.\.\d+$/,
    'Star filter must be in format: ">100", ">=10", "<50", "10..100", or just "50"'
  )
 ...
 */
export const SizeFilterSchema = z
  .string()
  .regex(
    /^(>|>=|<|<=)?\d+$|^\d+\.\.\d+$/,
    'Size filter must be in format: ">1000", ">=100", "<500", "100..1000", or just "500"'
  )
  .transform(filter => filter.replace(/[^\d.<>=.]/g, ''));
...
export const DateFilterSchema = z
  ...
  .regex(
    /^(>|>=|<|<=)?\d{4}-\d{2}-\d{2}$/,
    'Date filter must be in format: ">2020-01-01", ">=2020-01-01", "<2023-12-31", or just "2020-01-01"'
  )
  .refine(dateFilter => {
...
export const CrossRepoProjectViewSchema = z.object({
  ...
  assignee: z
    .string()
    .max(39)
    .refine(validateOwnerName, 'Invalid assignee username format')
    .optional(),
  state: IssueStateSchema.optional(),
  milestone: z
    .string()
    .max(255)
    .transform(milestone => sanitizeText(milestone, 255))
    .optional(),
});
  ...
  repositories: z
    .array(
      z.object({
        ...
        alias: z
          .string()
          .regex(/^[a-zA-Z][a-zA-Z0-9_]*$/, 'Alias must be a valid identifier')
          .optional(),
      })
    )
    .min(1)
...
export function validateGraphQLVariableValue(value: any, variableName: string): any {
  if (typeof value === 'string') {
    ...
    if (value.includes('${') || value.includes('#{') || value.includes('{{')) {
      throw new GraphQLValidationError(
        variableName,
        'String contains potential injection patterns',
        'INJECTION_RISK'
      );
    }
    // Sanitize the string
  ...
  if (Array.isArray(value)) {
    if (value.length > 100) {
      throw new GraphQLValidationError(
        variableName,
        'Array too large (max 100 items)',
        'ARRAY_TOO_LARGE'
      );
    }
    return value.map((item, index) =>
      validateGraphQLVariableValue(item, `${variableName}[${index}]`)
    );
  }
  ...
  if (typeof value === 'object') {
    ...
    if (keys.length > 50) {
      throw new GraphQLValidationError(
        variableName,
        'Object has too many properties (max 50)',
        'OBJECT_TOO_COMPLEX'
      );
    }
  ...
  throw new GraphQLValidationError(
    variableName,
    `Unsupported variable type: ${typeof value}`,
    'UNSUPPORTED_TYPE'
  );
}
